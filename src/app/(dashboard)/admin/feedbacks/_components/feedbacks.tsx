...
import { HEADER_TABLE_FEEDBACKS } from "@/constants/feedbacks-constant";
import { format } from "date-fns";
import { FeedbacksData } from "@/types/feedbacks";
...
export default function ContentRequestManagement() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<FeedbacksData | null>(
    null
  );
  ...
  const {
    ...
    queryKey: ["feedbacks", currentPage, currentLimit, currentSearch],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (
      contentRequests?.count !== undefined &&
      contentRequests.count !== null
    ) {
  ...
  const handleViewDetail = (request: FeedbacksData) => {
  ...
  const filteredData = useMemo(() => {
    return (contentRequests?.data || []).map((request, index) => {
      const contentRequest = request as unknown as FeedbacksData;

      return [
        currentLimit * (currentPage - 1) + index + 1,
        contentRequest.subject,
        format(new Date(contentRequest.submission_date), "dd MMM yyyy HH:mm"),
        <div
          className={cn(
